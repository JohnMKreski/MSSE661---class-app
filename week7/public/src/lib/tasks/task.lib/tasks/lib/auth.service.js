...
class AuthService {
  ...
  setExpiration = (maxExpiration) =>
    new Date(new Date().getTime() + maxExpiration * 1000); //maxExpiration = 24hrs x 1000sec
...
const authService = new AuthService(); //initializes the class (new x) into authservice.X