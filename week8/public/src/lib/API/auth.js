const doLogin = async (e) => {
  ...
  try {
    ...
    if (res) {
      if (userRole === 'Admin') {
        window.location.href = 'src/lib/admin/admin-dashboard.html';
      } else {
        window.location.href = 'src/lib/song_req/song_req.html';
      }
    }
  } catch (err) {
...
(async () => {
  ...
  const usernameAdmin = document.getElementById('header-usernameAdmin');
  ...
  const setRegularUserDisplay = () => {
    ...
    // toggleDisplay(username, user.username);
  };
