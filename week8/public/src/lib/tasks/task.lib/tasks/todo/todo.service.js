...
/**
 * _x = internally refrenced functions
 * x = externally refrenced functions 
 */

class ToDo {
  tasks = [];//will always have an empty array until init is called
  tasksService; //from tasks-api.service.js

  constructor(tasksService) {
    this.tasksService = tasksService; //stores it to make it avaible within the app
  } //made tasksService neccessary when doing the todo.service

  init() {
    this.render();
  }//init calls the render where render gets the tasks
  ...
  _renderList = () => {
    ...
    const fragment = document.createDocumentFragment();//batch updates the DOM
  ...
  _createMsgElement = (msg) => {
    ...
    return msgDiv;
  };//block element to put custom messages into 
  ...
  render = async () => {
    ...
    try {
      if (tasks.length) {
        this.tasks = tasks; //need to have tasks present and store tasks into instance provided by this class(tasks = [])
        this._renderList();
      } else {
        this._renderMsg();
      }
  ...
  }; //loads up the tasks and focuses on one concern (display(render) something to me)
}